...
export async function verifyCredentials(username: string, password: string): Promise<boolean> {
  ...
  if (!env.ADMIN_USERNAME || !env.ADMIN_PASSWORD_HASH || !password) return false;
  ...
  return usernameMatches && constantTimeEqual(derived, decodeBase64(parts[3]));
}

export async function createSession(username: string, role: AppRole): Promise<string> {
